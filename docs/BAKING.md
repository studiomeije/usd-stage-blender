...
| Mesh UV maps | a generated `usdstage_bake` map, only where §3.7 needs one; the active UV map during the bake passes | yes; the generated map is removed after export |
...
| Node group whose Group Output ends in one Principled BSDF, with Alpha 1 | accepted, as that Principled | accepted |
| Node group ending in a Principled BSDF with Alpha below 1 or linked | refused | refused |
| Anything else: another shader, another node group, a reroute, nothing | refused | refused |

Reroutes between a node group's Principled BSDF and its Group Output, and
nested groups, are followed. Opacity inside a node group is not baked, so a
transparent one is refused rather than flattened.
...
**Sheen Weight**, **Subsurface Weight**, or emission (a non-zero **Emission
Strength** with a non-black **Emission Color**), because the rebuilt material keeps none of them. **IOR** and **Diffuse
...
Baked base color is `sRGB`; baked roughness and opacity are `Non-Color`. When
nothing reads the base image's alpha (an opaque bake, or one whose opacity has
its own texture), the rebuilt material reads it `CHANNEL_PACKED`. Cycles then
hands its color as stored rather than multiplied by alpha, and the image
exports as a plain image node feeding the surface (see
[`MATERIAL_TRANSLATION.md`](MATERIAL_TRANSLATION.md#image-color-and-alpha) for how straight-alpha
images are read otherwise).
...
The bake writes into the active UV map (`mesh.uv_layers.active`, falling back
to the first), unless that map cannot hold it. `_bind_uv_layer` wires an
explicit UV Map node into every image node on the baked material; an
unconnected `Vector` samples whatever map the renderer picks. Passed-through
textures keep reading the UV map named on their own node, or the active UV map.

#### When the bake generates a UV map

A bake gives each face the texels its UVs cover. Faces whose UVs leave the 0–1
square wrap onto texels other faces cover, and overlapping faces cover the same
texels, so those faces share one baked colour. `_resolve_bake_uv_layer` decides,
per object and per material (each material bakes into its own image), whether
that is acceptable:

| Bake mode | Material | Faces may share texels |
|---|---|---|
| `LIT_IBL` | any | **no**: lighting and shadows differ from face to face |
| `UNLIT_ALBEDO`, `LIT_ALBEDO` | reads only Image Textures set to Repeat and Flat projection, on the active UV map, through math, colour and whole-number Mapping nodes | yes: every face reads the colour its UVs point at |
| `UNLIT_ALBEDO`, `LIT_ALBEDO` | anything else: a procedural texture, Generated or Object coordinates, a colour attribute, another UV map, a node not listed above | **no** |
| any | flat (see below) | not baked |

Normal, Coat Normal and Tangent chains do not count, because a colour bake does
not record them.

When a material needs texels of its own, `bake_uv.uv_layout_problem` draws
that material's faces into a 1024 × 1024 grid. If any UV leaves the 0–1 square,
or more than 0.1% of the covered texels are covered twice, the object gets a
new UV map, `usdstage_bake`:

- It starts as a copy of the active UV map. Only the faces of the materials
  that failed are unwrapped with Smart UV Project, each material on its own
  into the whole square, with islands two bake margins apart.
- The render UV map and the active UV map do not change, so `st` and every
  passed-through texture keep your layout. The baked textures read
  `usdstage_bake`, which exports as its own primvar and is read through
  `geompropvalue` (see
  [`MATERIAL_TRANSLATION.md`](MATERIAL_TRANSLATION.md#uvs-and-texture-transforms)).
- The export warns: `'<object>': UV map '<map>' cannot hold the bake (material
  '<material>' leaves the 0-1 UV square), so those materials bake into the
  generated UV map 'usdstage_bake'.`
- Objects sharing a mesh share its generated map. The map is removed from the
  mesh when the bake's materials are restored after export.

The check reads the mesh's own UVs, before modifiers. A Mirror or Array
modifier that stacks copies onto the same UVs is not detected.
...
| A UV map cannot hold the bake (§3.7) | **succeeds with a warning**; the bake writes into a generated `usdstage_bake` UV map |
| A UV map cannot hold the bake and none can be generated: the mesh is linked from another `.blend`, or already has 8 UV maps | `Bake failed: on '<object>', UV map '<map>' cannot hold the bake (…)`; baked slots rolled back |
