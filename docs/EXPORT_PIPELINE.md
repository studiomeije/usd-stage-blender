...
import those schemas. Visibility follows Blender's own `evaluation_mode`
(`RENDER` by default); USD Stage for Blender does not filter on `hide_viewport` or
`hide_render`.
...
is on, and all scene materials otherwise. See [Error codes](CLI.md#error-codes)
for the full list.
